            ...
            <div className="main-tile">
                ...
                <nav className="navbar navbar-expand-lg">
                    {this.getPermissions().map((acct) => (
                          ...
                          <div>
                          <button type="button" className="btn btn-outline-info" key={acct[0]} onClick={()=>this.handleSelection(acct[0])}>Student #{acct[0]}</button>
                          &nbsp;&nbsp;
                          </div>
                    ))}
                </nav>
                ...
                <div className="card-body">
                    <h4 className="card-title">Post transcript data for: Student #{this.state.selectedAcct}</h4>
                    <p className="card-text">Student #{this.state.selectedAcct} selected, enter grade data and click Upload.</p>
                    <div className="form-inline my-5 my-lg-0 upload-box">
                    <input className="form-control my-sm-5" type="search" placeholder="Enter grades to add to transcript" aria-label="Upload" id="inputTxtBox" type="textbox"></input>
                    <button className="btn btn-primary" onClick={()=> this.putToServer()}>Upload</button>
                    </div>
                </div>
                </div>
